        ...
        {businesses.length > 0 ? (
                ...
                <div key={business.id} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 group">
                  ...
                  <div className="h-56 relative overflow-hidden">
                    ...
                    <div className="absolute top-4 right-4 z-10">
                      <span className={`px-2 py-1 rounded-md text-[10px] uppercase font-black shadow-lg ${business.opening_hours?.toLowerCase().includes('close')
                    ...
                    <div className="absolute bottom-4 left-4 z-10">
                      <span className="bg-yellow-400 text-black px-2 py-1 rounded-md text-[10px] uppercase font-black shadow-lg">
                        Destacado
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-6">
                    ...
                    {/* Price - Hidden if not provided as per Item 11 */}
                    {business.price && parseFloat(business.price.toString()) > 0 && (
                      <div className="flex items-center mb-4">
                        <span className="text-lg font-bold text-orange-600">
                          ${Number(business.offer_price || business.price).toLocaleString('es-CO')}
                        </span>
                        {business.has_offer && business.offer_price && (
                          <span className="ml-2 text-sm text-gray-400 line-through">
                            ${Number(business.price).toLocaleString('es-CO')}
                          </span>
                        )}
                      </div>
                    )}
                    ...
                    <div className="flex justify-between items-center">
                      {onBusinessClick ? (
                        <button
                          ...
                          className="bg-orange-600 text-white px-6 py-2.5 rounded-lg hover:bg-orange-700 transition-all duration-200 font-bold shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                        >
                          Ver Detalles
                        </button>
                      ) : (
                      ...
                      <button
                        ...
                        className={`flex items-center space-x-2 transition-colors duration-200 group ${isFavorite(business.id)
                          ? 'text-red-500'
          ...
          <div className="text-center py-16">
            <div className="max-w-md mx-auto">
              ...
              <Link
                href="/add-listing"
                className="bg-orange-600 text-white px-6 py-3 rounded-lg hover:bg-orange-700 transition-all duration-200 font-bold shadow-md hover:shadow-lg"
              >
                Agrega Tu Negocio
              </Link>
            </div>
